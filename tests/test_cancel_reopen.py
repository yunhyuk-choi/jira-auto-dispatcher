...
def test_job_cancel_requested_roundtrips():
    j = Job(ticket="HAN-1", cancel_requested=True)
...
def test_cancel_queued_job_drops_and_releases_dedup(isolated_state):
    ...
    gate.claim("HAN-2")
    sch = _sch(gate=gate)
    sch.enqueue(Job(ticket="HAN-1", user="u1", target_repos=["repoA"]))
    sch.enqueue(Job(ticket="HAN-2", user="u2", target_repos=["repoA"]))  # 같은 레포 대기
    assert sch.jobs.get("HAN-2").status == q.QUEUED

    sch.cancel_job("HAN-2")  # 큐 대기 → 즉시 드롭
    assert sch.jobs.get("HAN-2").status == q.CANCELLED
    assert gate.is_claimed("HAN-2") is False          # dedup 해제
    assert sch.jobs.get("HAN-1").status == q.RUNNING  # 무관 잡 영향 없음
...
def test_cancel_running_marks_cancelling_and_holds_lock(isolated_state):
    ...
    sch = _sch(gate=gate)
    sch.enqueue(Job(ticket="HAN-1", user="u1", target_repos=["repoA"]))
    sch.enqueue(Job(ticket="HAN-2", user="u2", target_repos=["repoA"]))
    assert sch.jobs.get("HAN-1").status == q.RUNNING

    sch.cancel_job("HAN-1")  # 실행 중 → worker에 위임(회신 대기)
    j = sch.jobs.get("HAN-1")
    assert j.status == q.CANCELLING
    assert j.cancel_requested is True
    # 레포 락 유지(cancelling이 점유) → HAN-2 아직 대기.
    assert sch.jobs.get("HAN-2").status == q.QUEUED
    # dedup은 회신 전까지 유지.
...
def test_confirm_cancelled_releases_lock_and_dedup(isolated_state):
    ...
    sch.report("HAN-1", "취소됨", rolledback=True)
    assert sch.jobs.get("HAN-1").status == q.CANCELLED
    assert gate.is_claimed("HAN-1") is False              # dedup 해제(재오픈 대비)
    assert sch.jobs.get("HAN-1").meta.get("rolledback") is True
    # 락 해제 → 대기하던 HAN-2 dispatch.
    assert sch.jobs.get("HAN-2").status == q.RUNNING
...
def test_cancel_terminal_job_is_noop(isolated_state):
    ...
    assert sch.cancel_job("HAN-1") == []             # 이미 종결 → no-op
...
def test_reopen_reenqueues_same_ticket(isolated_state):
    ...
    gate.claim("HAN-1")
    sch = _sch(gate=gate)
    sch.enqueue(Job(ticket="HAN-1", user="u1", target_repos=["repoA"], session_id="old"))
    sch.cancel_job("HAN-1")
    sch.report("HAN-1", "취소됨")               # cancelled + dedup 해제
    assert sch.jobs.get("HAN-1").status == q.CANCELLED

    sch.reopen(Job(ticket="HAN-1", user="u1", target_repos=["repoA"]))
    j = sch.jobs.get("HAN-1")
...
def test_control_reports_cancel_flag(isolated_state):
    ...
    disp.enqueue("u1", Job(ticket="HAN-1", target_repos=["repoA"]))
    assert disp.control("u1", "HAN-1") == {"cancel": False}
    sch.cancel_job("HAN-1")                       # running → cancelling + flag
    assert disp.control("u1", "HAN-1") == {"cancel": True}
    with pytest.raises(PermissionError):
        disp.control("other", "HAN-1")
...
def test_http_control_route(isolated_state):
    ...
    assert client.get("/dispatch/u1/HAN-1/control").status_code == 401  # 인증 없음
    r = client.get("/dispatch/u1/HAN-1/control", headers={"X-Worker-Secret": "secret"})

...
def make_client(responder):
    c = JiraClient("https://interx-jira.atlassian.net", "yh.choi@interxlab.com", "tok")
...
def test_search_jql_pagination_follows_next_page_token():
    pages = [
        {"issues": [{"key": "HAN-1"}], "nextPageToken": "t2"},
        {"issues": [{"key": "HAN-2"}], "nextPageToken": "t3"},
        {"issues": [{"key": "HAN-3"}], "isLast": True},  # 마지막
    ]
    seq = {"i": 0}
    bodies = []
    ...
    result = c.search_jql("project = HAN", fields=["status"], max_results=1)
    assert [i["key"] for i in result["issues"]] == ["HAN-1", "HAN-2", "HAN-3"]
...
def test_transition_posts_correct_path_and_body():
    ...
    c = make_client(responder)
    c.transition("HAN-5", "41", fields={"customfield_x": "v"})
    assert captured["method"] == "POST"
    assert captured["url"].endswith("/rest/api/2/issue/HAN-5/transitions")
...
def test_transition_done_prefills_actual_dates():
    ...
    c = make_client(responder)
    c.transition_done("HAN-9", "2026-08-01", "2026-08-10")
...
def test_get_issue_and_comment_paths():
    ...
    def responder(method, url, kwargs):
        ...
        return FakeResponse(200, {"key": "HAN-1"})

    c = make_client(responder)
    c.get_issue("HAN-1", fields=["status", "assignee"])
    c.add_comment("HAN-1", "한글 코멘트")
    assert seen[0][0] == "GET" and seen[0][1].endswith("/rest/api/2/issue/HAN-1")

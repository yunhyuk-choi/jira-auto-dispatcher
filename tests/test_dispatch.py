...
def test_next_job_returns_running_for_user(isolated_state):
    ...
    assert got is not None and got.ticket == "HAN-1"
...
def test_report_status_terminal_triggers_on_complete(isolated_state):
    _, sch, disp = _wire()
    disp.enqueue("u1", Job(ticket="HAN-1", target_repos=["repoA"]))
    disp.enqueue("u1", Job(ticket="HAN-2", target_repos=["repoA"]))  # 같은 레포 대기
    assert sch.jobs.get("HAN-2").status == q.QUEUED
    disp.report_status("u1", "HAN-1", {"status": "완료", "mr_url": "http://mr"})
    assert sch.jobs.get("HAN-1").status == q.DONE
    assert sch.jobs.get("HAN-1").mr_url == "http://mr"
    assert sch.jobs.get("HAN-2").status == q.RUNNING   # 완료-구동 다음 dispatch


def test_report_status_interrupted_records_reset_at(isolated_state):
    _, sch, disp = _wire()
    disp.enqueue("u1", Job(ticket="HAN-1", target_repos=["repoA"]))
    disp.report_status("u1", "HAN-1", {"status": "interrupted", "reset_at": "2099-01-01T00:00:00Z"})
    j = sch.jobs.get("HAN-1")
...
def test_cross_user_claim_rejected(isolated_state):
    ...
    disp.enqueue("u1", Job(ticket="HAN-1", target_repos=["repoA"]))
    try:
        disp.report_status("other", "HAN-1", {"status": "완료"})
...
def test_http_next_and_status_with_auth(isolated_state):
    ...
    assert r.status_code == 200 and r.get_json()["ticket"] == "HAN-1"
    # 다른 유저는 204
    assert client.get("/dispatch/nobody/next", headers={"X-Worker-Secret": "secret"}).status_code == 204
    # status 회신
    r = client.post("/dispatch/u1/HAN-1/status", headers={"X-Worker-Secret": "secret"},
                    json={"status": "완료"})
